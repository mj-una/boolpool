...
/*
  ...
  Este es un juego para 2 personas. Cada una lanza una bola en su turno e intenta
  caturar al paso las dos manzanas de la pantalla, o bien capturar 4 manzanas en 
  total. Si una bola es empujada por la otra y captura la manzana, no se cuenta
  como punto válido. Al finalizar cada movimiento se cambia de turno y, en caso
  de corresponder, se reponen las manzanas en sus nuevas posiciones. 
  ...
  La idea original es mia (al menos que yo sepa). Se me ocurrio mientras intentaba
  crear un juego de tiro al blanco para un trabajo practico de la universidad. Una
  vez tenga un mejor codigo y todas las funcionalidades implementadas, lo compartire
  en github bajo licencia CC no comercial.
  ...
  ni de una plataforma en especifico, y se puede adaptar a todo tipo de situaciones.
  *Inspirado en idea de Olia Lialina en "Usuaria Turing Completa" (2012) 
  https://endefensadelsl.org/usuaria_turing_completa.pdf

  Link a repositorio proyecto completo:
  https://github.com/mj-una/BoolPool
  
  Link a futura pagina web (v0.5):
  https://boolpool.github.io/
  ...
  los centros de las dos particulas en colision. Las velocidades en y se mantienen
  constantes ya que inciden perpendicularmente y no afectan ni es afectadaas por
  las otras fuerzas. Las velocidades en x se resuleven como una colision de una
  sola dimmension. En este caso las masas de las bolas es la misma y no cambia por
  lo que el calculo se reduce a intercambiar las velocidades de una particula a la
  ...
  Resuelto: las colisiones de mucha velocidad que se dan en un determinado angulo
   ...
   solucion: lineas 421 a 441.
   
  Resuelto: velocidad minima = entre 0 y radio, aprox radio (tamBola * 2 / 5)
   solucion: linea 332.
   
  Pendiente: posicion p0 y p1, velocidades p0 y p1, posicion manzanas, posicion
   areas de obstruccion (esquinas canvas), posicion mouse --> pasar a vectores.
   Reescribir codigo
   
  Pendiente: array de boolean para manzanas activas y array de boolean para
   obstrucciones de areas (distancia player - esquina). Reescribir codigo.
   
  Pendiente: mecanismo para evitar que las manzanas que estan reapareciendo lo
   hagan en area obstruida por una bola. Prioridad: 1 evitar obstruccion, 2 evitar
   usar area recientemente ocupada, 3 permitir azar (o decision). 
   
  Pendiente: bolas son objetos. Posicion, velocidades y angulo mediante get y set.
   
  Pendiente: botones son objetos. Parametros para constructor: texto, pantalla en
   que se encuentra y pantalla a la que lleva. Contiene propio draw, mouse pressed
   y mouse released. 1 boton principal (seguir) y 2 secundarios (volver y opciones).
   
  Pendiente: dibujo cursor es objeto? Contiene propio draw, contador desva.,
   mouse moved.
   
  Pendiente: limpiar draw (pasar bloques a funciones externas).
   
  PLAN v0.4

  -Navegabilidad en menu. info / ajustes. atras / no mostrar de nuevo.
  -Pantalla pausa: reanudar, ajustes, menu (salir-nmdn).
  -Estructura final del codigo (objetos, arrays, vectores, funciones). Renombrar
  ...
  -Sistema de turnos mediante referencias 1B2D. 10 numeros para contar de 0 a
   100 (asocia cob map() distanciaM) y 19 letras: para contar de 0 a 360 (asocia
   con grados anguloM). Ademas se incluye un boolean para determinar posicion nueva
   manzana (si hubo eleccion): natural o salto --> numero antes de letras o letras
   antes de numero. Cada referencia 1B2D informa un lanzamiento unico entre los
   72.000 (10*19*10*19*2) posibiles y comunicables.
...
void draw() {
  ...
  if (pantalla == 0) { // ----------------------------------- [0]
    ...
    text("Bool Pool v0.3.1", width / 2, height / 12);
    ...
    textFont(arial_d15);
    textSize(width / 24);
    text("Bool Pool v0.3.1 (22-5-23)", width / 2, height * 7.75 / 12);
